...
class ModelDataBarangkeluar extends Model
{
    ...
    protected $column_order = array('', 'faktur', 'tglfaktur', 'nama', 'totalharga', '');
    protected $column_search = array('faktur', 'tglfaktur');
    ...
    private function _get_datatables_query($tglawal, $tglakhir)
    {
        ...
        if ($tglawal == '' && $tglakhir == '') {
            $this->dt = $this->db->table($this->table)->join('pelanggan', 'id = idplgn', 'left');
        } else {
            $this->dt = $this->db->table($this->table)->join('pelanggan', 'id = idplgn', 'left')
                ->where('tglfaktur >=', $tglawal)->where('tglfaktur <=', $tglakhir);
        }
        $i = 0;
    ...
    {
        if ($tglawal == '' && $tglakhir == '') {
            $tbl_storage = $this->db->table($this->table)->join('pelanggan as p', 'p.id = idplgn', 'left');
        } else {
            $tbl_storage = $this->db->table($this->table)->join('pelanggan as p', 'p.id = idplgn', 'left')
                ->where('tglfaktur >=', $tglawal)->where('tglfaktur <=', $tglakhir);
        }
